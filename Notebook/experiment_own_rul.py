...
RUL_own = dict(zip(df_own.engine_id, df_own.RUL_predicted))
...
for i in range(N_RUNS):
    best_ind_own, best_cost_own, hist = run_ga(rul_input=RUL_own, seed=i)
    all_histories_own.append(hist)
    all_bests_own.append((best_ind_own, best_cost_own))
    print(f"Run {i:2d} | Best cost: {best_cost_own:,.0f}")
    # Save full history list after every run so progress is not lost on crash
    _max_len = max(len(h) for h in all_histories_own)
    _padded  = [h + [h[-1]] * (_max_len - len(h)) for h in all_histories_own]
    np.save("Results/histories_own_rul.npy", np.array(_padded))

# Overall best individual across all 30 runs
overall_best_ind_own, overall_best_cost_own = min(all_bests_own, key=lambda x: x[1])

# Save best schedule to txt matching the print_schedule format from the notebook
with open("Results/best_schedule_own_rul.txt", "w", encoding="utf-8") as f:
    ...
    maintained = {}
    for engine_id, team_id, start_day in overall_best_ind_own:
        duration = get_maintenance_duration(engine_id, TEAM_TYPES[team_id])
        end_day  = start_day + duration - 1
        due_date = get_engine_due_date(engine_id, RUL_own)
    ...
    for engine_id in range(1, 101):
        due = get_engine_due_date(engine_id, RUL_own)
...
padded_own = [h + [h[-1]] * (max_len_own - len(h)) for h in all_histories_own]

# Average best fitness per generation across all runs
avg_fitness_own = np.mean(padded_own, axis=0)
...
plt.show()
print("Saved Results/convergence_own_rul.png")
